 ...
 *   GET  /reports/status/{requestId}       inspect one queued report
...
class ReportJobController extends Controller
{
    ...
    /**
     * Return the state of one report request without scanning the user's latest
     * notifications. The request ID is generated by the server and every query
    ...
    public function status(Request $request, string $requestId): JsonResponse
    {
        ...
        $notification = Notification::query()
            ...
            ->whereIn('action', ['report_ready', 'report_failed'])
        ...
        return response()->json([
            'status' => $notification->action === 'report_ready' ? 'ready' : 'failed',
            'notification' => [
                'id' => $notification->id,
    ...
    public function download(Notification $notification): StreamedResponse
    {
        ...
        // New reports record their exact disk. Reports generated before that
        // metadata existed fall back to the report disk rather than the global
        // upload disk, which may point at storage this app no longer owns.
        $storageDisk = $meta['storage_disk'] ?? config('filesystems.reports', 'local');

        // A disk that can no longer be built (rotated or removed credentials)
        // means the file is unreachable — that is a 410, not a 500.
        try {
        ...
        abort_if(!$exists, 410, 'This report has expired or is no longer available.');
